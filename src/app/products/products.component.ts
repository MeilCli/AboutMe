...
export class ProductsComponent implements OnInit, OnDestroy {
    ...
    private setProducts(products: Product[]) {
        ...
        this.types = [...new Set(products.map((x) => x.type).sort((x, y) => x.localeCompare(y)))];
        this.relationShipToProducts = [
            ...new Set(products.map((x) => x.relationShipToProduct).sort((x, y) => x.localeCompare(y))),
        ];
    ...
    relationShipToProductFilter(relationShipToProduct: string) {
        this.products = this.productsSource.filter((x) => x.relationShipToProduct === relationShipToProduct);
    }
